...
export default function Categories() {
    ...
    const loadData = async () => {
        try { const res = await categoryService.getAll(); setCategories(res.data); } catch (e) { console.error(e); }
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            editing ? await categoryService.update(editing.id, formData) : await categoryService.create(formData);
            setIsModalOpen(false); setEditing(null); setFormData({ name: '', code: '', description: '' }); loadData();
        } catch (e) { console.error(e); alert('Error al guardar'); }
    };

    const handleDelete = async (id) => {
        if (window.confirm('¿Eliminar esta categoría?')) {
            try { await categoryService.delete(id); loadData(); } catch (e) { console.error(e); }
        }
    };
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div><h1 className="text-3xl font-bold text-gray-900">Categorías</h1><p className="text-gray-600 mt-1">Gestión de categorías de activos</p></div>
                <Button onClick={() => { setEditing(null); setFormData({ name: '', code: '', description: '' }); setIsModalOpen(true); }}><MdAdd className="inline mr-2" />Nueva Categoría</Button>
            </div>
            <Card>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={editing ? 'Editar Categoría' : 'Nueva Categoría'}>
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div><label className="block text-sm font-medium mb-1">Código</label><input type="text" value={formData.code} onChange={(e) => setFormData({ ...formData, code: e.target.value })} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-primary-500" /></div>
